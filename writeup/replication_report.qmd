...
pilot_A_raw <- read_csv(here("data", "pilot_A_raw.csv"))
...
# plot of pilot A raw data
typeof(pilot_A$hire)
typeof(pilot_A_raw$hire)
pilot_A_raw = pilot_A_raw |> 
  mutate(hire = as.numeric(hire))
typeof(pilot_A_raw$hire)

ggplot(data = pilot_A_raw,
       mapping = aes(x = distorted_audio,
                     y = hire,
                     color = distorted_audio)) +
  geom_beeswarm(cex = 10) +
