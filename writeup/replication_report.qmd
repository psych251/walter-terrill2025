...
pwr_replication = pwr_replication %>% 
  mutate(n_per_group = c(62, 62, 125, 125),
         total_n = 250,
...
In addition to the hireability question, participants will respond to the following question from the original study's Experiment 4, which used the same script and audio manipulation: "How intelligent is the author of this personal statement?" The scale will also be continuous, from 0 ("Very Unintelligent) to 100 ("Very Intelligent"). The order of the questions will be counterbalanced. Finally, participants will answer the following follow-up questions during debriefing:
...
Finally, to generate hypotheses for future directions, I will investigate (1) within-person correlations between hireability and intelligence judgements.
...
This code anonomyzes the data file and removes participants according to the exclusion critera used in the original study: self-reported attention \< 70/100, technical difficulties or distractions, response times \> \$\pm\$2.5 SD than the mean, and comprehension scores \$\pm\$2 SD from the mean.
...
model_hire = lm(hire ~ audio_quality + voice_type + audio_quality*voice_type,
           data = pilot_B)
summary(model_hire)
```
...
### Exploratory analyses {#sec-exploratory}
...
model_intel = lm(intelligence ~ audio_quality + voice_type + audio_quality*voice_type,
           data = pilot_B)
summary(model_intel)
```
```{r}
...
```{r}
# within-person coupling: are hire and intelligence ratings correlated?
...
ggplot(pilot_B, aes(x = hire, y = intelligence)) +
  ...
  labs(
    title = "Hireability and Intelligence Ratings",
